...
### Host
The core Superfluid contract that manages all agreements and protocol operations. Acts as the central coordinator for all Superfluid functionality.

### Forwarder
Simplified contract interfaces that make common Superfluid operations easier. The SDK primarily uses:
...
### Flow NFT
An NFT that represents an active flow. The NFT holder receives the streamed tokens.
...
### Batch Call
A Superfluid feature allowing multiple operations in a single transaction. Useful for complex workflows.
