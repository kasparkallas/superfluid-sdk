...
```ts
import { cfaV1ForwarderAbi } from "@superfluid-finance/sdk/abi";
```
...
import { readCfaForwarderGetFlowInfo } from "@superfluid-finance/sdk/actions";
...
const { flowrate } = await readCfaForwarderGetFlowInfo(publicClient, {
  token:      "0xSUPER",
  sender:     "0xSENDER",
  receiver:   "0xRECEIVER"
});
...
import { readCfaForwarderGetFlowInfo } from "@superfluid-finance/sdk/wagmi";
...
function FlowInfo() {
  const { data, isLoading } = readCfaForwarderGetFlowInfo({
    superToken: "0xSUPER",
    sender:     "0xSENDER",
    receiver:   "0xRECEIVER",
  });
